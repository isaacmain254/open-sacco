...
import { LoanStatus } from "@/services/loans";
...
const LoansView = () => {
  ...
  const [modal, setModal] = useState<"approve" | "reject" | "disburse" | null>(null);
  ...
  const runAction = async (actionName: "submit" | "review" | "approve" | "reject" | "disburse", payload: Record<string, string> = {}) => {
    try {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <Modal isOpen={modal !== null} onClose={() => setModal(null)} title={modal === "approve" ? "Approve application" : modal === "reject" ? "Reject application" : "Disburse loan"}><div className="space-y-4">{modal === "disburse" && <select className="h-10 w-full rounded border px-3 text-slate-900" value={accountNumber} onChange={(event) => setAccountNumber(event.target.value)}><option value="">Select member account</option>{accounts.filter((account) => account.is_active).map((account) => <option key={account.account_number} value={account.account_number}>{account.account_number} — balance {money(account.balance)}</option>)}</select>}<textarea className="min-h-24 w-full rounded border p-3 text-slate-900" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder={modal === "reject" ? "Rejection reason (required)" : "Notes (optional)"} /><Button disabled={action.isPending || (modal === "reject" && !notes.trim()) || (modal === "disburse" && !accountNumber)} variant={modal === "reject" ? "destructive" : "default"} onClick={() => modal === "approve" ? runAction("approve", { approval_notes: notes }) : modal === "reject" ? runAction("reject", { rejection_reason: notes }) : runAction("disburse", { account_number: accountNumber, disbursement_notes: notes })}>{modal === "approve" ? "Confirm approval" : modal === "reject" ? "Confirm rejection" : "Confirm disbursement"}</Button></div></Modal>
    </div>
  );
};

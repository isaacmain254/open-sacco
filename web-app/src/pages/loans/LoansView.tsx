...
const LoansView = () => {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {loan.loan_account && <section className="rounded-lg border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-medium">Repayment schedule</h2><p className="mt-1 text-sm text-slate-500">{loan.loan_account.interest_type === "flat" ? "Flat" : "Reducing-balance"} interest at {loan.loan_account.interest_rate}%.</p></div><div className="text-right text-sm"><div>Amount repayable: <strong>{money(loan.loan_account.total_repayable)}</strong></div><div>Outstanding: <strong>{money(loan.loan_account.outstanding_balance)}</strong></div></div></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="border-b text-slate-500"><tr><th className="pb-2">Installment</th><th className="pb-2">Due date</th><th className="pb-2">Principal</th><th className="pb-2">Interest</th><th className="pb-2">Amount due</th><th className="pb-2">Status</th></tr></thead><tbody>{loan.loan_account.schedule.map((item: LoanInstallment) => { const overdue = !item.is_paid && new Date(`${item.due_date}T00:00:00`) < new Date(new Date().toDateString()); return <tr key={item.installment_number} className="border-b last:border-0"><td className="py-2">{item.installment_number}</td><td className="py-2">{new Date(`${item.due_date}T00:00:00`).toLocaleDateString()}</td><td className="py-2">{money(item.principal_due)}</td><td className="py-2">{money(item.interest_due)}</td><td className="py-2 font-medium">{money(item.total_due)}</td><td className="py-2"><Badge variant={item.is_paid ? "default" : overdue ? "destructive" : "secondary"}>{item.is_paid ? `Paid${item.paid_at ? ` · ${new Date(item.paid_at).toLocaleDateString()}` : ""}` : overdue ? "Overdue" : "Due"}</Badge></td></tr>; })}</tbody></table></div></section>}
